...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, hidden_size, total_key_depth, total_value_depth, filter_size, num_heads,
        ...
        self.multi_head_attention = RandomMultiHeadAttention(hidden_size, total_key_depth, total_value_depth,
                                                       hidden_size, num_heads, bias_mask, attention_dropout)

        self.positionwise_feed_forward = RandomPositionwiseFeedForward(hidden_size, filter_size, hidden_size,
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, hidden_size, total_key_depth, total_value_depth, filter_size, num_heads,
        ...
        self.multi_head_attention_dec = RandomMultiHeadAttention(hidden_size, total_key_depth, total_value_depth,
                                                           hidden_size, num_heads, bias_mask, attention_dropout)

        self.multi_head_attention_enc_dec = RandomMultiHeadAttention(hidden_size, total_key_depth, total_value_depth,
...
class RandomMultiHeadAttention(nn.Module):
    ...
    def __init__(self, input_depth, total_key_depth, total_value_depth, output_depth,
        ...
        """
        super(RandomMultiHeadAttention, self).__init__()
        ...
        self.query_linear1 = nn.Linear(input_depth, rand_num, bias=False)
        self.key_linear1 = nn.Linear(input_depth, rand_num, bias=False)
        self.value_linear1 = nn.Linear(input_depth, rand_num, bias=False)

        self.query_linear1.weight.requires_grad = False
        self.key_linear1.weight.requires_grad = False
        self.value_linear1.weight.requires_grad = False

        self.query_linear2 = nn.Linear(output_depth+rand_num, total_key_depth, bias=False)
        self.key_linear2 = nn.Linear(output_depth+rand_num, total_key_depth, bias=False)
        self.value_linear2 = nn.Linear(output_depth+rand_num, total_value_depth, bias=False)

        self.output_linear1 = nn.Linear(total_value_depth, rand_num, bias=False)
        self.output_linear1.weight.requires_grad = False
        self.output_linear2 = nn.Linear(total_value_depth+rand_num, output_depth, bias=False)
    ...
    def forward(self, queries, keys, values, mask):
        torch.nn.init.xavier_normal_(self.query_linear1.weight, gain=1.0)
        torch.nn.init.xavier_normal_(self.key_linear1.weight, gain=1.0)
        torch.nn.init.xavier_normal_(self.value_linear1.weight, gain=1.0)

        # Do a linear for each component
        Rqueries = self.query_linear1(queries)
        Rkeys = self.key_linear1(keys)
        Rvalues = self.value_linear1(values)

        comb_queries = torch.cat([Rqueries, queries], dim=2)
        comb_keys = torch.cat([Rkeys, keys], dim=2)
        comb_values = torch.cat([Rvalues, values], dim=2)

        queries = self.query_linear2(comb_queries)
        keys = self.key_linear2(comb_keys)
        values = self.value_linear2(comb_values)
        ...
        torch.nn.init.uniform_(self.output_linear1.weight, a=0.0, b=0.1)
        outputs = self.output_linear1(contexts)
        comb = torch.cat([outputs,contexts], dim=2)
        outputs = self.output_linear2(comb)
        return outputs, attetion_weights

class RandomPositionwiseFeedForward(nn.Module):
    """
    ...
    def __init__(self, input_depth, filter_size, output_depth, layer_config='ll', padding='left', dropout=0.0):
        ...
        Parameters:
            ...
            dropout: Dropout probability (Should be non-zero only during training)
        """
        super(RandomPositionwiseFeedForward, self).__init__()

        layers = []
        sizes = ([(input_depth, filter_size)] +
                 [(filter_size, filter_size)] * (len(layer_config) - 2) +
                 [(filter_size, output_depth)])
        self.linear1 = nn.Linear(input_depth, filter_size)
        self.linear1.requires_grad = False
        self.linear2 = nn.Linear(filter_size+input_depth, output_depth)
        #self.linear2 = nn.Linear(filter_size, output_depth)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(dropout)

    def forward(self, inputs):
        x = inputs
        torch.nn.init.xavier_uniform_(self.linear1.weight, gain=nn.init.calculate_gain('relu'))
        x = self.linear1(x)
        x = self.relu(x)
        x = self.dropout(x)
...
def evaluate(model, data, ty='valid', max_dec_step=50):
    ...
    for j, batch in pbar:
        loss, ppl, bow_prog = model.train(batch, 0, train=False)
        l.append(loss)
        p.append(ppl)
        bow.append(bow_prog)
    ...
    print("{}\t{:.4f}\t{:.4f}\t{:.2f}\t{:.4f}\t{:.4f}\t{:.4f}".format(ty, loss, math.exp(loss), bleu_score_g, gd1, gd2,
                                                                      gd3))

    return loss, math.exp(loss), bow, gd1, gd2, gd3
