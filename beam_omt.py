...
class Beam():
    ...
    def advance(self, word_prob):
        ...
        prev_k = torch.floor(best_scores_id / num_words)
        #if 5. in prev_k:
            #prev_k[prev_k == 5.] = 4.
        #print(prev_k)
        ...
        self.next_ys.append(torch.nn.functional.relu(best_scores_id - prev_k * num_words, inplace=True))
        #print(torch.nn.functional.relu(best_scores_id - prev_k * num_words, inplace=True))
    ...
    def get_hypothesis(self, k):
        ...
        for j in range(len(self.prev_ks) - 1, -1, -1):
            #print(self.next_ys[j+1].size())
            #print([k])
            hyp.append(self.next_ys[j+1][k.long()])
    ...
    def translate_batch(self, src_seq):
        ...
        def beam_decode_step(inst_dec_beams, len_dec_seq, src_seq, enc_output, inst_idx_to_position_map, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src):
            ...
            def predict_word(dec_seq, dec_pos, src_seq, enc_output, n_active_inst, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src):
                ...
                #print(self.model.embedding(dec_seq).size())
        ...
        with torch.no_grad():
            ...
            src_enc,_ = self.model.encoder(self.model.embedding(enc_batch),mask_src)

            #--Random Encode
            rand_src_enc, _ = self.model.rand_encoder(self.model.embedding(enc_batch), mask_src)

            src_enc = torch.concat([src_enc, rand_src_enc], dim=2)
